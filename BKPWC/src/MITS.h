...
//		edge.resize(n, vector<double>(n));						// 分配空间，重设大小
		...
		// 从大到小排序（降序）
		sort(ord_edge.begin(), ord_edge.end(), [](const EdgeNode &a, const EdgeNode &b){
...
//		cout << "\nfilename = " << filename << endl;
		...
		// step 1：设置固定下来的参数
		tau = 2;
		int num_edge = 0; // 边的数量(临时接收一下，实际上没用到)
		int cur_id = 0;										// 记录当前点id
		cur_level = 0;										// 初始化当前级别为0
		cg_dist = 0.0;
		...
		if (!iss.fail()) {  // 是大图的格式
			...
			for (int i = 0; i < k; i++)
			{
				iss >> temp_lower >> temp_upper;

				if (i > 0 && (FL != temp_lower || FU != temp_upper)) {
					cerr << "Error: Instance " << filename << " has different partition limits! error in partition_"<< i <<endl;
				}
				else if (i == 0){
					FL = temp_lower;
					FU = temp_upper;
			...
			// 遍历权重
			iss >> cluster_type; // 先把字符W读出来
			cout << "读取开始 " << cluster_type << endl;
			cout << "FileName = " << filename << endl;
			cout << "Number of vertex :" << n << ", Number of edge : " << num_edge << endl;
			...
			// 1.读取每个点的权重
			// step 3：读取每一个点的信息（点权重，邻接边）
			for (int i = 0; i < n; i++)
			{
				vector<double> attrs(tau);
				iss >> attrs[0]; // 每个点的权重
				attrs[1] = 1; // 代表个数
				Vertex vtx = Vertex(i, attrs); // 生成点
				vertices[i] = vtx; // 将点加入到点集中
			}
			...
			while (getline(fin, line))
			{
				...
				double w;
				iss >> vi >> vj >> w;
				edge[vi][vj] = edge[vj][vi] = w;
				if (w > max_dist) max_dist = w;
			}
		}
		else {
			// step 2. 读取第一行：点数和k值
			...
			cout << "FileName = " << filename << endl;
			cout << "Number of vertex :" << n << ", k : " << k << ", Number of edge : " << num_edge << endl;
			if (n <= 3 || n >= 10000 || k <= 1 || k >= 5000)
				cout << "Out of bounds!" << endl;
...
// 更新vtx_attr_diff
...
#if(READW)
        if (type == 0) {
            w_lower[0] = ((double) n / (double) k) * mu - sigma;
            w_upper[0] = ((double) n / (double) k) * mu + sigma;
        }
        else {
            w_lower[0] = FL;
            w_upper[0] = FU;
        ...
        cout << "Weights: mu = " << mu << ", sigma = " << sigma << ", WL = " << w_lower[0] << ", WU = " << w_upper[
                    0] <<
                "." << endl;
        ...
        if (rest != 0) {
            ...
            for (int i = 0; i < nn; i++) dd[i] = new double[nn];
...
#endif
#endif
		cout << "Weights: mu = " << mu << ", sigma = " << sigma << ", WL = " << w_lower[0] << ", WU = " << w_upper[0] <<
				"." << endl;
...
//		print_sol(graph);
	...
	void cal_dist(Graph &graph)
	{
		...
		for (int i = 0; i < graph.n; i++)
		{
			...
			{
				// 距离相关
				dist_vtx_to_cluster[i][ptn[j]] = dist_vtx_to_cluster[i][ptn[j]] + graph.edge[i][j];
				dist_vtx_to_cluster[j][ptn[i]] = dist_vtx_to_cluster[j][ptn[i]] + graph.edge[i][j];

				if (ptn[i] == ptn[j])
					dist = dist + graph.edge[i][j];
			}
		}
		dist = myRound(dist);
	}
	...
	void cal_infeasible(Graph &graph)
	{
		...
		for (int i = 0; i < graph.k; i++)
		{
			for (int t = 0; t < graph.tau; t++)
			{
				...
				// 加和
				ifsb = ifsb + cluster_infeasible[i];
			}
		}
	}

	/*
	 * 将sol2中的关键信息复制到当前sol中 TODO:判断哪些内容不用留
	 */
...
//vector<int> aux_match;								// 剩余未匹配的 TODO:考虑把aux_match定义为动态数组
...
int seed = 14;
char filename[1000] = "./Instances/MDG-selected/MDG-a_21.40.txt";
string graph_name;
double tmax = 10;

/* =============================================调参的参数============================================= */
int max_lvl = 1;										// 压缩到的最大层级
//const double so_rate = 0.1;												// 战略震荡的范围
double pct_mcc = 0.05;											// 每一层合并的点数
double tt_pct = 0.9;													// tabu tenure
double td_pct = 0.75;													// tabu depth
...
//	for(int i = 0; i < graph.tau; i++)
//		t_ifsb_upper[i] = so_rate*(max_t[i] - min_t[i]);
...
//	ils_best_sol = Solution(graph);				// 在函数里面定义（涉及对折叠图的搜索）
//	ts_best_sol = Solution(graph);				// 在函数里面定义（涉及对折叠图的搜索）
...
void coarsen_process(Graph &graph)
{
	...
	while (cur_lvl < max_lvl && coarsen_graph[0].n > 60)
	{
		cout <<"第" << cur_lvl<<"层压缩" <<endl;
		/* 1.重置参数，初始化*/
		// 初始化
...
//		cout << "测试合并成功否1:" << new_cg.vertices[0].pre_vts[0];
//		cout << "测试合并成功否2:" << new_cg.vertices[0].pre_vts[1];
	...
	for(int i = 0; i < graph.n; i++)
	{
		...
		for (int j = 0; j < graph.n; j++)
		{
			if (sol.ptn[i] != sol.ptn[j])
			{
				...
				ifsb_change = myRound(ic1 + ic2 - sol.cluster_infeasible[move_clst1] - sol.cluster_infeasible[move_clst2]); 						// 如果是infeasible下降阶段就不是-0了 TODO：这里需要减什么？
...
/*
 * 初始化（如果前面保证了infeasible相关的参数正确的情况下，）删掉 TODO
 */
...
void cost_tabu(Solution &cur_sol, Graph &graph, int *move_flag)
{
	...
	ts_iter = 0;
	int non_improve = 0;
	while ((clock() - start) / (double)CLOCKS_PER_SEC < tmax)
	{
		non_improve++;

		// 验证
//		if(min_c_gain.type == 1){
//			cout << "\n\n迭代轮次：" << ts_iter << endl;
//			cout << "本轮移动elem1：" << min_c_gain.elem1 <<endl;
//			cout << "本轮移动elem2：" << min_c_gain.elem2 <<endl;
//			cout << "本轮移动clst1：" << cur_sol.ptn[min_c_gain.elem1] <<endl;
//			cout << "本轮移动clst2：" << cur_sol.ptn[min_c_gain.elem2] <<endl;
//			cout << "本轮移动类型：" << min_c_gain.type <<endl;
//			cout << "move gain值：" << min_c_gain.move_gain << endl;
//			cout << "两个点的距离" << graph.edge[min_c_gain.elem1][min_c_gain.elem2] << endl;
//			cout << "elem1到clst2的距离" << cur_sol.dist_vtx_to_cluster[min_c_gain.elem1][cur_sol.ptn[min_c_gain.elem2]] << endl;
//			cout << "elem1到clst1的距离" << cur_sol.dist_vtx_to_cluster[min_c_gain.elem1][cur_sol.ptn[min_c_gain.elem1]] << endl;
//			cout << "elem2到clst1的距离" << cur_sol.dist_vtx_to_cluster[min_c_gain.elem2][cur_sol.ptn[min_c_gain.elem1]] << endl;
//			cout << "elem2到clst2的距离" << cur_sol.dist_vtx_to_cluster[min_c_gain.elem2][cur_sol.ptn[min_c_gain.elem2]] << endl;
//			cout << "cluster_attr[clst1][0]:" << cur_sol.cluster_attrs[cur_sol.ptn[min_c_gain.elem1]][0] << endl;
//			cout << "cluster_attr[clst2][0]:" << cur_sol.cluster_attrs[cur_sol.ptn[min_c_gain.elem2]][0] << endl;
//			cout << "vtx_attr_diff[elem1][elem2][0]:" << graph.vtx_attr_diff[min_c_gain.elem1][min_c_gain.elem2][0] << endl;
//			cout << "vtx_attr_diff[elem2][elem1][0]:" << graph.vtx_attr_diff[min_c_gain.elem2][min_c_gain.elem1][0] << endl;
//			cur_sol.verify_cluster_attrs(graph, "tabu更新之后");
//			cur_sol.verify_ifsb_change(graph, "tabu更新之后");
...
//		update_tabu_move_gain3(cur_sol, graph, min_c_gain);
//		update_tabu_move_gain(cur_sol, graph, min_c_gain);			 
#if(DEBUG_cost_tsso)
	printf("\n after——type=%d ifsb_change=%f cur_ifsb=%f", min_c_gain.type, min_c_gain.ifsb_mg, cur_sol.ifsb);
	fflush(stdout);
...
//		cout << "ILS_iter=" << iter << endl;	// DEBUG
//		infeasible_descent(sol, graph);	// 改进不可行程度
		// infeasible_tabu(sol, graph);
